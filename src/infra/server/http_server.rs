...
impl Default for HyperHttpServer {
    fn default() -> Self {
        HyperHttpServer::new(|_c,_r|async{
            Ok(Response::new(Body::from(r#"{"health":true}"#)))
        })
    }
}
...
impl HyperHttpServer {
    ...
    pub fn append_handles(mut self, mut filters:Vec<Box<dyn HttpFilter+Send+Sync+'static>>) ->Self{
        self.filters.append(&mut filters);self
    }
    pub async fn service(self:Arc<HyperHttpServer>,req:Request<Body>) ->Result<Response<Body>, Infallible>{
        let ctx = Context::new();
        let mut index = 0;
        ...
        if req.is_some(){
            ...
            resp = match result {
                ...
                Err(e) => {
                    ...
                    Some(Response::builder().status(500).body(Body::from("unknown error")).unwrap())
                }
            };

        }
        let skip = self.filters.len() - index - 1;
        for (_,filter) in self.filters.iter().rev().enumerate().filter(|(i,_)|{i>= &skip}) {
...
impl HyperHttpServerBuilder {
    pub fn new()->Self{
        ...
        let handle = None;
        let shutdown = None;
        Self{addr,filters,handle,shutdown}
    }
    #[allow(dead_code)]
    ...
    pub fn shutdown_singe(mut self)->(Self,ShutDown){
        ...
        self.shutdown = Some(sdc);(self,sd)
    }
    pub fn set_shutdown_singe(mut self,sd:ShutDown)->Self{
        self.shutdown = Some(ShutDownControl::from(sd));self
    }
    pub fn handle<H:HttpHandle + Send + Sync + 'static>(mut self,handle:H)->Self{
        self.handle = Some(Box::new(handle));self
    }
    #[allow(dead_code)]
    pub fn parse_addr<S:ToSocketAddrs<Iter=SocketAddr>>(mut self,addr:S)->anyhow::Result<Self>{
        self.addr = addr.to_socket_addrs()?;Ok(self)
    }
    pub fn append_filter<H:HttpFilter+Send+Sync+'static>(mut self,filter:H)->Self {
        self.filters.push(Box::new(filter));self
    }
    pub async fn custom_run<C:FnOnce(Builder<AddrIncoming>)->anyhow::Result<Builder<AddrIncoming>>+Send>(self,custom_func:C)->anyhow::Result<()>
    {
        if self.handle.is_none() {
            return Err(anyhow::anyhow!("HyperHttpServerBuilder handle function is nil,please impl HttpHandle"))
        }
        let ser = Arc::new(HyperHttpServer::from(self.handle.unwrap()).append_handles(self.filters));
        let http_service = make_service_fn(move |_c:&AddrStream|{
            let ser = ser.clone();
            let service = service_fn(move |req| {
                HyperHttpServer::service(ser.clone(), req)
            });
            async move {
    ...
    pub fn async_run(self)->ShutDown{
        ...
        tokio::spawn(async move{
            server.custom_run(|b|{
                Ok(b)
            }).await.expect("async_run http server error")
        });
        return sd;
    }
}