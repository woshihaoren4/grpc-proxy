...
impl TestExampleBuilder{
    pub fn new()->Self{
        Self{examples: HashMap::new()}
    }

    pub fn init_examples(mut self) ->Self {
        self.examples.insert("dyn".into(),Box::new(TestExampleDynClient));
        self
    }
    pub fn build(self)->TestExample{
        TestExample{examples:Arc::new(self.examples)}
    }
}
...
impl TestExample {
    pub fn args(&self) -> CmdInfo {
        ...
        CmdInfo::new("test", "test example").add(
            "m",
            "all",
            format!("test models: {}",ms),
        )
    }
}
...
impl wd_run::EventHandle for TestExample {
    fn handle(&self, ctx: Context) -> Pin<Box<dyn Future<Output=Context> + Send>> {
        ...
        Box::pin(async move {
            ...
            let ods = order.split(",").map(|x|x.to_string()).collect::<Vec<String>>();
            wd_log::log_info_ln!("start test");
            if order.eq("all"){ //测试全部
...
impl wd_run::EventHandle for TestExampleDynClient {
    fn handle(&self, ctx: Context) -> Pin<Box<dyn Future<Output=Context> + Send>> {
        Box::pin(async move {
            let index = FileDescProfiler::new().map("127.0.0.1:666".into(), SimpleIndex::parse).await.expect("parse grpc index from reflect failed");
            wd_log::log_info_ln!("FileDescProfiler reflect success");
            let client = DynClient::new(JsonProtoTransitionDefaultImpl, index).set_host_port("127.0.0.1:666");
            let request_body = Vec::from(r#"{"request":"hello"}"#);
            let (_,resp) = client.invoke(Method::POST,"/api/v2/hello".into(), HashMap::new(), request_body).await.expect("invoke grpc request failed");
            let resp_body = String::from_utf8_lossy(resp.as_slice()).to_string();
            wd_log::log_info_ln!("response body:{}",resp_body);
            assert_eq!(resp_body,r#"{"response": "hello world"}"#,"grpc response body is error");
            return ctx
        })
    }
}